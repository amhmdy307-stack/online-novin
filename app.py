...
SMS_API_KEY = "7fnvBUIylbYOg1MYdApEj2sCVggrHnyf89qNMmAx3BTM8cf6"
SMS_SENDER = "30002101009128"
...
def send_sms_smsir(mobiles, message):
    """
    ...
    try:
        if not mobiles:
            print("SMS ERROR: شماره موبایل وجود ندارد.")
            return False
        ...
        for mobile in mobiles:
            ...
            if mobile and mobile not in clean_mobiles:
                clean_mobiles.append(mobile)

        if not clean_mobiles:
            print("SMS ERROR: هیچ شماره موبایل معتبری وجود ندارد.")
            return False

        payload = {
            "lineNumber": SMS_SENDER,
            "MessageText": str(message),
            "Mobiles": clean_mobiles
        }
...
def send_request_sms(request_row, message):
    """
    ارسال پیامک همزمان به مشتری و مدیر
    """
    ...
    try:
        ...
        if "customer_phone" in request_row.keys():
            customer_phone = request_row["customer_phone"]

        settings = get_settings()

        manager_phone = settings.get(
            "phone",
            ""
        )
        ...
        if manager_phone:
            numbers.append(manager_phone)

        if not numbers:
            print(
                "SMS ERROR: شماره مشتری و مدیر خالی است."
            )
            return False

        return send_sms_smsir(
            numbers,
            message
        )
...
def send_customer_sms(request_row, message):
    """
    ارسال پیامک فقط برای مشتری پرونده
    """
    ...
    try:
        ...
        if not customer_phone:
            print(
                "SMS ERROR: شماره مشتری وجود ندارد."
            )
            return False
...
def create_tables():
    ...
    for d in days:
        ...
        if not conn.execute(
            "SELECT id FROM work_hours WHERE day_name=?",
            (d,)
        ).fetchone():
            ...
            conn.execute(
                ...
                (day_name, start_time, end_time, is_rest)
                VALUES (?, '09:00', '18:00', 0)
                """,
                (d,)
            )
...
def create_request(service_id):
    ...
    if discount_code:
        ...
        if d:
            ...
            if (
                d["start_date"]
                and today < d["start_date"]
            ):
                valid = False

            if (
                d["end_date"]
                and today > d["end_date"]
            ):
                valid = False
    ...
    # ========================================================
    # SMS - ثبت درخواست جدید
    # پیام برای مشتری + مدیر
    # ========================================================
    ...
    flash(
        f"درخواست ثبت شد. کد پیگیری شما: {tracking}",
        "success"
    )
...
def admin_integrations_save():
    ...
    flash(
        "تنظیمات پیامک و درگاه ذخیره شد.",
        "success"
    )
...
def admin_request(rid):
    ...
    if (
        ...
        flash(
            "این پرونده به کارشناس دیگری اختصاص دارد.",
            "error"
        )
    ...
    if request.method == "POST":
        ...
        # ====================================================
        # SMS - تغییر وضعیت + توضیحات کارشناس
        # مشتری + مدیر
        # ====================================================
...
def admin_request_update():
    ...
    if sms_row:
        ...
        sms_message = (
            "کافی نت آنلاین نوین\n"
            "اطلاعات پرونده شما به‌روزرسانی شد.\n"
            f"کد پیگیری: {sms_row['tracking_code']}"
        )
...
def save_work_hours():
    ...
    for day_id in request.form.getlist(
        "day_id"
    ):
...
def admin_backup():
    ...
    flash(
        f"پشتیبان‌گیری انجام شد: novin_manual_{ts}.db",
        "success"
    )
...
def server_error(e):
    ...
    return (
        "خطای داخلی سرور. لطفاً لاگ را بررسی کنید.",
        500
    )
